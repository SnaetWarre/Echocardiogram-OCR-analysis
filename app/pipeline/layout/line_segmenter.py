...
class LineSegmenter:
    ...
    *Horizontal* extent per line: tight bbox from `_text_mask` ink columns in that stripe
    ...
    def _segment_fixed_pitch(self, content: np.ndarray, *, header_trim_px: int) -> list[SegmentedLine]:
        ...
        if runs and len(runs) <= estimated_line_count:
    ...
    def _estimate_line_count(self, content_height: int) -> int:
        if content_height <= 0:
            return 0
        estimated = int(np.floor(float(content_height) / self.target_line_height_px))
    ...
    def _segment_from_projection(self, content: np.ndarray, *, header_trim_px: int) -> list[SegmentedLine]:
        ...
        if not runs:
            return []
        estimated_line_count = self._estimate_line_count(int(content.shape[0]))
