...
def _load_checkpoint(path: Path) -> tuple[list[dict[str, Any]], set[str]]:
    ...
    try:
        ...
        try:
            payload = json.loads(raw_text.replace("\\", "\\\\"))
        except Exception:
            return [], set()
    ...
    processed = {
        _canonical_path(Path(str(item.get("dicom_path", "")).strip()))
...
def _scoped_resume_state(
    ...
    scoped_items = [
        ...
        if str(item.get("dicom_path", "")).strip()
        and _canonical_path(Path(str(item.get("dicom_path", "")).strip())) in discovered_keys
    ]
...
def _build_pipeline(args: argparse.Namespace) -> EchoOcrPipeline:
    ...
    config = PipelineConfig.with_parameters(parameters)
